...
# Сколько живёт карта «воркспейс/вкладка → метка» для адреса панели в
# уведомлении. Метки меняются редко (переименование вкладки), а платить за них
# двумя вызовами herdr на каждое уведомление незачем.
HERDR_PLACE_TTL = _int_env("PP_HERDR_PLACE_TTL", 30)
