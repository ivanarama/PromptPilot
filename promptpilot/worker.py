...
def execute_task(task):
    """Run the task, then extend its schedule whatever the outcome was.

    Концов у прогона полтора десятка (шесть видов mark_failed, ранние return,
    detached-путь), и продление расписания жило только на двух успешных из них.
    Поэтому оно вынесено в finally: серия переживает и падение, и внутреннюю
    ошибку воркера.
    """
    try:
        _execute_task(task)
        ...
        except Exception as e:  # расписание не должно валить сам прогон
            print(f"  !! не смог продлить расписание #{task.id}: {e}", flush=True)
