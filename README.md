...
Кнопка **«📈 Обновить анализ очереди OneBase»** и команда бота `/pipeline`
считают внешний backlog через GitHub CLI и сопоставляют его с ёмкостью одного
прогона. Узкое место — этап с максимальным `backlog / capacity`. Встроенный
профиль OneBase можно переопределить общим конфигом
`~/.promptpilot/pipeline_profiles.json` (или путём из `PP_PIPELINE_PROFILES`),
поэтому сам анализатор остаётся частью PromptPilot, а метки и лимиты проекта —
данными профиля.
...
`series_contains`. Несколько проектов — это несколько профилей; Web UI покажет
их в выпадающем списке. Пользовательский файл по умолчанию:
