...
MERGE также завершается без модели с `action=wait`, если свежий проектный health
однозначно показывает владельца интеграционной полосы на `integration-review`
или `legacy-integration-review`: единственный REVIEW-кандидат совпадает по PR,
HEAD и этапу, `single_flight_barrier` относится к нему, а `merge_executable`
явно пуст. Причина ожидания и PR сохраняются в результате preflight. Старый,
неполный или противоречивый формат health использует прежний маршрут; готовый
к MERGE владелец и recovery продолжают полную процедуру. Незавершённый
`merge-cleanup-intent` обрабатывается раньше этой проверки. Кэш дашборда
не используется для этого решения, проверки перед мутациями не меняются.
