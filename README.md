...
Для распознанной серии конвейера контракт дополнительно разрешает `УСТАРЕЛО` —
отдельный безопасный исход: точная цель, HEAD или
её executable-позиция изменились **до первой мутации**. Это не `НЕ СМОГ` и не
ошибка здоровья. Следующее вхождение серии создаётся сразу, чтобы выполнить
свежий election; в дашборде такие перевыборы считаются отдельно. Неоднозначный
отказ, проблема доступа и ошибка после начатой мутации по-прежнему должны
заканчиваться `НЕ СМОГ` или `НУЖЕН ЧЕЛОВЕК`, а не быстрым циклом перевыбора.
