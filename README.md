...
- **`--dangerously-skip-permissions`** — флаг на задачу для запуска Claude без интерактивных подтверждений (Web UI + бот)
- **Скилы Claude Code** — запуск `/skill-name` через Web UI и бота (только для Claude-провайдеров)
...
### Режим 2 — CLI (как раньше, все команды работают)
...
| ➕ Добавить задачу | Пошаговое создание: промпт → провайдер → приоритет → директория → skip-permissions |
...
Цепочка не ограничена: каждый "ответ" тоже получает кнопку 💬, можно отвечать сколько угодно раз. Новая задача наследует провайдера, рабочую директорию и флаги оригинальной задачи.
...
Скилы — это команды (`/skill-name`) из `~/.claude/commands/` и плагинов Claude Code. Работают **только** с провайдерами `claude` и `claude-z`; для Qwen Code и Codex недоступны.

### Web UI

При выборе Claude-провайдера под полем промпта появляется кнопка **⚡ Skills**. Нажми — откроется список скилов с описаниями. Выбор скила подставляет `/skill-name ` в промпт — дополни аргументами если нужно и отправь как обычно.
...
Показывает список скилов с описаниями. Нажми кнопку скила → бот спросит аргументы (если скил их принимает) или сразу перейдёт к выбору приоритета → директория → расписание → задача создана.
...
```json
[
  {"name": "commit", "description": "Create a git commit", "argument_hint": "", "source": "plugin:commit-commands"},
  {"name": "feature-dev", "description": "Guided feature development", "argument_hint": "Optional feature description", "source": "plugin:feature-dev"}
]
```

Скилы из `workdir/.claude/commands/` добавляются к глобальным при наличии рабочей директории.
...
- Чекбокс `--dangerously-skip-permissions` — запустить Claude без интерактивных подтверждений
- **⚡ Skills** — кнопка появляется автоматически при выборе Claude-провайдера; раскрывает список доступных скилов из `~/.claude/commands/` и плагинов; выбор скила подставляет `/skill-name` в поле промпта
- Фильтры по статусу
...
GET    /api/providers          — список провайдеров с флагом supports_skills
GET    /api/skills             — список скилов (?provider=claude&workdir=/path)
GET    /api/projects           — список проектов из PP_PROJECTS_ROOT
```
...
| `PP_TASK_PASSWORD` | — | Пароль для создания задач через бота (не задан — без защиты) |
