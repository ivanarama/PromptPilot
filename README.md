...
`claude.exe` на Windows). Переопределяется через `PP_CLAUDE_EXE`.
...
- обычный — панель закрывается после задачи, результат в базе;
- `"keep_pane": true` (или флаг `--keep-pane`) — результат сохраняется, но
  **сессия остаётся открытой**: приходите, читаете транскрипт и продолжаете
  диалог в той же панели;
...
- **OpenCode** — флаг `--variant high|max|minimal` есть только у headless-режима
  (`opencode run`): добавьте его в `cmd`-шаблон. В интерактивной (herdr) сессии
  флага запуска нет — вариант переключается внутри сессии.
...
- В списке провайдеров показываются только установленные и не скрытые
  (`pp provider hide <name>`)
...
| `PP_HERDR_KEEP_PANE` | `0` | Оставлять панель после успешной herdr-задачи |
