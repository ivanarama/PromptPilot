...
- **Per-task таймаут** — задать индивидуальный лимит времени для задачи в Web UI и боте (переопределяет глобальный `PP_TASK_TIMEOUT`)
...
| ➕ Добавить задачу | Промпт → провайдер → **модель** → приоритет → skip-permissions → директория → расписание → повтор → **режим запуска** |
