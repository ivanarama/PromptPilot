...
- **Итог задачи** — `PP_VERDICT=1`: агент заканчивает строкой `ИТОГ: ГОТОВО | НУЖЕН ЧЕЛОВЕК | НЕ СМОГ`, и уведомление сразу говорит, надо ли идти смотреть
...
- идущий прогон её уже не увидит: хочешь применить сейчас — отмени задачу
  (`pp cancel 42`), она вернётся в очередь и возьмётся с припиской;
...
```
ИТОГ: ГОТОВО | УЖЕ СДЕЛАНО | НУЖЕН ЧЕЛОВЕК | НЕ СМОГ
```
