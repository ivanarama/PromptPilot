...
<p align="center">
  <img src="docs/screenshots/task-queue.png" alt="Очередь задач PromptPilot" width="960">
</p>
<p align="center"><sub>Одна очередь для разовых задач, расписаний и автономных workflow.</sub></p>
...
- **Автономный режим** — флаг на задачу для запуска Claude или Codex без интерактивных подтверждений
...
- **Повторяющиеся задачи** — поле Recur: `6h`, `30m`, `daily@09:00` — новая задача создаётся автоматически, в том числе после неудачного прогона
- **Экран расписания** — кнопка 🗓 в Web UI: все повторяющиеся задачи как серии — период, следующий запуск, исход прошлого и пометка «оборвана»
...
- **Отмена running-задач** — из Web UI, бота или API: worker убивает всё принадлежащее задаче дерево процессов в течение пары секунд
...
таймауте, отмене или аварийном завершении worker закрывает именно локальное
дерево этой задачи — включая `cmd`/npm-обёртки и их потомков — не затрагивая
соседние задачи и другие программы. Если создать такую границу не удалось,
задача не запускается (fail closed).
...
<p align="center">
  <img src="docs/screenshots/workflow-executor.png" alt="Workflow PromptPilot: сейчас работает исполнитель" width="960">
</p>
<p align="center"><sub>Экран сразу показывает, у кого «мяч» и какой переход произойдёт дальше.</sub></p>

<details>
<summary>Как устроен автономный цикл</summary>

<p align="center">
  <img src="docs/screenshots/workflow-cycle.png" alt="Цикл planner, executor, gate и независимого аудитора" width="960">
</p>
...
Если на машине установлена платформа 1С:Предприятие 8.3, в меню бота появляется кнопка **«🔧 1С обработка»** (и команда `/epf`): пришли `.epf`/`.erf`, текст задачи и базу — PromptPilot сам декомпилирует обработку в XML-проект (git + cc-1c-skills), отдаст задачу агенту, а по завершении соберёт и вернёт готовый `.epf`.

То же самое доступно в веб-интерфейсе — кнопка **«🔧 1С»**: создание доработки (файл + текст + база) и редактор настроек (пишет секцию `"1c"` в `tg_config.json`, действует сразу; переменные `PP_1C_*` сильнее настроек из формы). Задачи из `/epf` помечены в списке бейджем 🔧 со статусом сборки.

Настройка (`.env` или секция `"1c"` в `tg_config.json`):

```ini
PP_1C_EXE_PATH=C:\Program Files\1cv8\8.3.25\bin\1cv8.exe   # иначе — реестр/Program Files
PP_1C_BASES_ROOT=C:\1c-bases                               # сканирование баз для выбора
PP_1C_BASE_PATH=C:\1c-bases\БППустаяДляСборки3_0_188_22    # база декомпиляции по умолчанию
PP_1C_UT_BASE=C:\1c-bases\УТПустаяДляСборки11_5_25_80      # пресеты: ut11/bp3/erp/unf/aa
PP_1C_SKILLS_DIR=C:\Users\me\.claude\skills\cc-1c-skills   # иначе клонируется сам
```

Без найденного `1cv8.exe` кнопка в меню не показывается — остальной PromptPilot не меняется.
...
- Поэтому отмена или таймаут задачи в чужой панели останавливает только ожидание
  PromptPilot: сам пользовательский агент может продолжить работу, и это явно
  записывается в результат задачи. Созданная самой задачей панель, напротив,
  закрывается с проверкой; неудачное закрытие считается ошибкой, а не успешной
  отменой.
...
### Расписание: серии, временное ускорение и анализ очереди

Повторяющаяся задача хранится как постоянная серия, а каждый запуск — отдельное
вхождение этой серии. Кнопка **🗓 Расписание** в Web UI показывает период,
параметры (провайдер, модель, эффорт, машина), следующий запуск, результат
прошлого прогона и статистику здоровья. Кнопка **«Изменить»** правит всю серию:
основной интервал, effort, приоритет и таймаут сохраняются для будущих запусков,
даже если текущий уже работает.
...
Для серий, входящих в настроенный pipeline-профиль, действует защита от
бессмысленного цикла: если два запуска подряд завершаются одним и тем же
объяснённым итогом `НУЖЕН ЧЕЛОВЕК` или `НЕ СМОГ`, PromptPilot ставит серию на
обычную паузу и не создаёт третье вхождение. При сравнении игнорируются только
служебные метаданные, SHA/UUID и номер самой scheduler-задачи; номера issue и PR
остаются частью причины, поэтому разные блокеры не склеиваются. После устранения
причины обычное «Возобновить» создаёт ровно одно новое вхождение.
...
показывают любой настроенный pipeline-профиль. Явное **«Обновить»** считает
внешний backlog через GitHub CLI, сопоставляет его с ёмкостью одного прогона и
...
прогонов за 5 и 24 часа, а также тренд backlog за 7 и 30 дней. Это обычные
GitHub API, SQLite и арифметика — LLM не
...
Команда бота `/pipeline` переиспользует тот же durable-снимок, поэтому её
обычное чтение не запускает отдельную принудительную диагностику GitHub. До
первого успешного фонового или явного обновления панель честно показывает
«нет снимка», не подменяя неизвестный backlog нулём.
...
Необязательный точный вектор `priority_one_headroom` сохраняет поверх аварийного
остатка запас для задач PromptPilot с приоритетом `1`: для задач приоритетов
`2`–`10` он прибавляется к `minimum_remaining`, а задача приоритета `1` может его
использовать. Полное обновление статистики считается низкоприоритетной работой и
тоже сохраняет этот запас. Ненулевой `priority_one_headroom` требует полной
таблицы `costs` во всех включённых профилях с тем же GitHub token; при общем
token PromptPilot покомпонентно применяет максимальный запас из этих профилей.
Это admission-резерв для срочного запуска, а не гарантия неизменности живого
лимита GitHub во время уже работающих задач.
...
задаче, ожидание остаётся прерываемым: освобождение точной резервации сразу
будит такие задачи, а `busy_retry_seconds` служит страховочным опросом после
...
PromptPilot по умолчанию переиспользует результат дорогой проверки 30 минут,
продолжая собирать снимки активных очередей каждые 5 минут. Кнопка «Обновить»
обходит оба кэша, пока worker не поставлен на общую паузу. `cache_seconds`
позволяет изменить период; ошибка checker
кэшируется не дольше минуты, чтобы временный отказ быстро восстановился. Панель
также показывает остаток и время сброса GitHub REST, Search и GraphQL budgets.
Общая пауза — строгий барьер GitHub I/O: sampler, завершение ранее запущенной
задачи и даже явное «Обновить» сохраняют последний снимок без нового опроса.
Текущие heartbeat, pause и статусы задач накладываются на него из локальной БД,
поэтому состояние worker остаётся актуальным. Команда
...
проектный CLI. PromptPilot не привязан ни к Claude, ни к Codex: он сам выполняет
безопасный preflight до запуска провайдера и только затем решает, нужна ли
модель. В очередь профиля добавляется:
...
REST health не доказывает timeline lineage: одинаковый снимок может означать
как штатное ожидание REVIEW, так и испорченный handoff, где MERGE обязан снять
устаревший `ship`. Поэтому такой снимок нельзя превращать в `action=wait` без
полного fallback-гейта.

Исключение — opt-in `base_sync_merge: true` в `pipelinectl.json`: владелец
барьера обслуживается детерминированно, когда его готовность доказана.
В состоянии `BEHIND` выдаётся lease `update-branch`: единственная мутация —
обновление ветки под CAS `expected_head_sha`, ответ проверяется до какого-либо
публикуемого факта (конфликт 422 ничего не публикует — урок ошибки из #42);
сам MERGE и `done`-маркер выполняются обычным путём на обновлённом HEAD.
В состоянии `CLEAN` владелец проходит те же proof/labels/CI-гейты, что и
обычный ship PR, и сливается с `base_sync_owner`-lease, чтобы барьер от его
собственного номера не ронял завершение. REST-only снимки (`merge_executable`
без списка) сохраняют полный fallback даже с opt-in.

Если для MERGE настроен более высокий приоритет и `priority_one_headroom`,
подписанный и привязанный к точной попытке `integration-review` временно
наследует приоритет активной MERGE-серии: этот REVIEW является зависимостью,
которая освобождает сам MERGE. Обычный content REVIEW запас не использует.
Временный приоритет хранится только у текущего вхождения серии, переживает
смену попытки при безопасном
budget-defer, снимается при смене маршрута, а следующий запуск повторяющейся
REVIEW-серии снова получает её настроенный приоритет.
...
| Статус | Значение |
|---|---|
| `green` — «работает штатно» | Инварианты соблюдены, активных ожиданий нет |
| `yellow` — «есть ожидания» | Выполняется восстанавливаемая транзакция или нужен ход человека; это не поломка |
| `red` — ошибка | Worker не работает, запуск превысил timeout, серия оборвана либо нарушен проектный инвариант |
...
<p align="center">
  <img src="docs/screenshots/pipeline-dashboard.png" alt="Дашборд внешнего конвейера в PromptPilot" width="1120">
</p>
<p align="center"><sub>Жёлтый статус означает ожидаемый handoff или ход человека; подробная причина видна в той же карточке.</sub></p>
...
Сейчас профиль и серии связываются по названию. Отдельного мастера «Создать
...
- **Claude Code и Codex** — уровень `low|medium|high|xhigh|max` задаётся полем
  и работает на двух уровнях:
  - **у провайдера** — «Эффорт рассуждений» в форме провайдера (⚙ Providers)
    или `pp provider add ... --effort max`. Это дефолт: с ним живут все задачи
    провайдера, в том числе этапы конвейеров;
  - **у задачи** — селект «Effort» рядом с «Model» в веб-мастере и кнопка
    «Эффорт» в разделе «Дополнительно» бота. Перекрывает провайдера на один
    прогон и наследуется следующими вхождениями повторяющейся задачи.

  Пустое значение = «по умолчанию»: PromptPilot не передаёт override и уровень
  выбирает сам CLI. Для Claude используется `--effort`, для Codex — одноразовый
  `-c model_reasoning_effort=...`; глобальная конфигурация агента не меняется.
  Если настройка уже вписана руками в шаблон команды или аргументы провайдера,
  выигрывает написанное руками. Остальным агентам effort не передаётся.
- **OpenCode** — два пути:
...
| `PP_PIPELINE_SNAPSHOT_INTERVAL` | `300` | Интервал фоновых снимков активных pipeline-профилей, сек; `0` отключает |
...
| `PP_GH_EXE` | стандартный путь/PATH | Путь к GitHub CLI; его каталог добавляется в окружение локальных агентов |
| `PP_GO_EXE` | стандартный путь/PATH | Путь к Go; его каталог добавляется в окружение локальных агентов |
...
| `PP_HERDR_WORKTREE_TIMEOUT_SECONDS` | `300` | Таймаут herdr-операций create/open/remove для worktree |
