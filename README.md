...
Команда бота `/pipeline` переиспользует тот же пятиминутный снимок, поэтому её
обычное чтение не запускает отдельную принудительную диагностику GitHub.
...
продолжая собирать лёгкие снимки очередей каждые 5 минут. Кнопка «Обновить»
обходит оба кэша. `cache_seconds` позволяет изменить период; ошибка checker
...
Поставленные на паузу профили не опрашиваются фоновым sampler. Команда
...
запуском провайдера PromptPilot использует общий снимок не старше пяти минут;
