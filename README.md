...
### Расписание: серии, обрыв и правка

Повторяющаяся задача — не один объект, а цепочка: очередное вхождение создаёт
следующее. Кнопка **🗓 Расписание** в Web UI собирает эту цепочку обратно в
серию по промпту и рабочей директории и показывает то, чего не видно в общем
списке: период, параметры (провайдер, модель, эффорт, машина), когда следующий
запуск, чем кончился прошлый прогон и сколько их было.
...
**В боте это тот же экран — кнопка 🗓 Расписание или `/schedule`.** Список серий с
периодом и следующим запуском, карточка серии с параметрами и исходом прошлого
прогона, а на ней кнопки: **▶️ Запустить сейчас** (сдвигает ближайшее вхождение
на «сейчас»), **✖️ Остановить** (отменяет его — серия дальше не идёт) и
**🔁 Возобновить** для оборванной (заводит новое вхождение с теми же
параметрами). Бот — интерфейс для телефона, то есть ровно для той ситуации,
когда за компьютером не сидят, а «оборвана» замечать надо именно тогда.

**Уведомление об обрыве.** Раз в `PP_SCHEDULE_WATCH_INTERVAL` (по умолчанию 15
минут) бот проверяет серии и пишет о той, у которой не осталось запланированного
вхождения: молчание бота иначе неотличимо от «всё в порядке, делать нечего». Об
одном обрыве говорится один раз; серия, остановленная кнопкой или руками, — это
решение человека, о ней сторож молчит. Уведомление уходит в чат последнего
прогона серии, и под ним сразу кнопка «🔁 Возобновить».

Задачу, которая ещё стоит в очереди (`pending`/`rate_limited`), можно править
прямо в карточке: провайдер, модель, эффорт, повтор, приоритет. До этого
поменять этап конвейера можно было только пересозданием задачи — с потерей
`tg_chat_id`, таймаута и времени следующего запуска. У `running` и завершённых
правка недоступна: там менять уже нечего, а следующее вхождение серии — это
отдельная строка, её и надо править.

### Эффорт (усилие рассуждений)

- **Claude Code** — уровень `low|medium|high|xhigh|max` задаётся полем, а не
  флагом в командной строке, и работает на двух уровнях:
  - **у провайдера** — «Эффорт (Claude Code)» в форме провайдера (⚙ Providers)
  ...
  Пустое значение = «по умолчанию»: PromptPilot не передаёт флаг вовсе и
  уровень выбирает сам CLI. Если `--effort` уже вписан руками в шаблон команды
  или в доп. аргументы провайдера, выигрывает написанное руками. Не-Claude
  агентам флаг не передаётся никогда — у них его нет.
- **OpenCode** — два пути:
...
Кнопка **⚡ Скилы** в главном меню или команда `/skills`. Список — глобальные скилы плюс скилы проекта: рабочая папка берётся из выбранного проекта, из текущего мастера, из прошлой задачи этого чата или из самой свежей записи в базе — то есть проектные скилы видны без всякой настройки, как и в вебе.

`📁 Скилы проекта...` показывается всегда: проекты из `PP_PROJECTS_ROOT`, а если переменная не задана — рабочие папки прошлых задач, у которых есть свои скилы. Если не нашлось ни того, ни другого, экран объясняет, что настроить, вместо того чтобы исчезнуть.

На шаге промпта в мастере можно просто напечатать `/имя-скила` — Telegram размечает такое сообщение как команду, и раньше оно молча пропадало. Команды самого бота (`/cancel`, `/skip`, `/tasks`, …) остаются командами.
...
GET    /api/config                 — серверные умолчания для UI ({tg_chat_id, tg_bot})
```

### Уведомления у задач не из Telegram

Задача, заведённая в вебе, из API или из CLI, не знает chat id — браузеру его
взять неоткуда. Без адресата молчит всё: вердикт прогона, «упала, но расписание
продолжено», «ждёт подтверждения в herdr» с кнопками, перенос по rate limit.
Поэтому:

- `PP_TG_CHAT_ID` — чат по умолчанию, подставляется задаче, которая своего не
  назвала. Узнать своё значение: команда **`/chatid`** в боте.
- Галочка **🔔 уведомлять в Telegram** в веб-форме (включена, если дефолт задан;
  если нет — на её месте объяснение, что настроить). Снятая галочка сохраняет
  `tg_chat_id = 0` — «эту задачу не уведомлять», и продление серии это уважает.
- `PATCH /api/tasks/<id>` принимает `tg_chat_id`: уже созданной серии можно
  назначить адресата, не пересоздавая её и не теряя историю прогонов. В карточке
  задачи это кнопка «Включить/Выключить» рядом со строкой «Уведомления».
  Менять можно, пока задача не завершилась, — адресат читается в момент доставки.
...
| `PP_TG_CHAT_ID` | — | Чат для уведомлений задач, заведённых не из Telegram (Web UI, API, CLI). Свой id — команда `/chatid` в боте |
...
| `PP_SCHEDULE_WATCH_INTERVAL` | `900` | Как часто бот проверяет, не оборвалась ли серия (сек); `0` — не проверять |
