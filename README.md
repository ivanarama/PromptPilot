...
Центральный PromptPilot может выполнять задачи на других машинах: каждая
машина — это провайдер через обёртку `scripts/pp-ssh-run`:

```bash
pp provider add claude-vm2 --desc "Claude на vm2" --cmd \
  "/path/to/scripts/pp-ssh-run vm2 /home/user/.local/bin/claude -p --verbose --output-format stream-json {prompt}"
```

Выбор машины = выбор провайдера в Web UI/боте. stream-json проходит через ssh
насквозь, поэтому стоимость, session_id и rate-limit-детекция работают как у
локального провайдера; флаги `--model`/`--resume`/`--dangerously-skip-permissions`
пробрасываются автоматически.

Требования: ssh по ключу (`BatchMode`, без парольных промптов), абсолютный
путь к CLI на удалённой машине (у неинтерактивного ssh куцый PATH). Команда
выполняется в домашней директории удалённой машины (рабочая директория задачи
локальна). Так же подключается codex/qwen/opencode — любой CLI.

### herdr → Telegram мост

Бот (если запущен) наблюдает за **всеми** агентами herdr на машине — не только
за задачами PromptPilot — и присылает уведомление, когда агент заблокирован
диалогом или закончил работу незамеченным. Кнопки под уведомлением:
**✅ Подтвердить (Enter)** · **📺 Экран** · **✍️ Ответить** — можно разблокировать
агента и переписываться с ним прямо с телефона. Отключение: `PP_HERDR_WATCH=0`.
...
POST   /api/tasks                  — создать задачу (все поля TaskCreate, вкл. keep_pane)
