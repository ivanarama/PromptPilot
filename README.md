...
завершается из Web UI и Telegram (`/schedule`).
...
снимки по `profile_id`. Узкое место — этап с максимальным `backlog / capacity`.
...
прогонов, а профиль задаёт желаемое время очистки (`target_clear_hours`, обычно
8 часов). Интервал выбирается из практичных пресетов. Оценка не обещает точную
дату и не учитывает новые входящие задачи; для этапа с `manual_gate: true`
ускорение не предлагается, потому что его ограничивает решение человека.
...
| `red` — «нарушен инвариант» | Автоматика обнаружила конфликт или не смогла выполнить обязательную проверку |
